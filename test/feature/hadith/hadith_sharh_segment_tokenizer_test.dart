...
void main() {
  ...
  group('Sharh 15844 — Tashahhud guillemet quotes', () {
    test('detects standalone guillemet quote segments', () {
      final segments = segmentsFor('15844');
      final quotes = segments
          .where((s) => s.kind == HadithSharhSegmentKind.quote)
          .map((s) => s.text)
          .toList();

      expect(quotes, contains('«التَّحيَّاتُ لله»'));
      expect(quotes, contains('«\u200fوالصَّلَواتُ»'));
      expect(quotes, contains('«\u200fوالطَّيِّباتُ»'));
