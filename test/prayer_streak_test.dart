...
void main() {
  ...
  group('Prayer streak calculation', () {
    ...
    Future<void> seedConsecutiveDays(int consecutiveDays, {int daysAgo = 0}) async {
    ...
    test('returns (1, 1) when only today is completed', () async {
      await seedConsecutiveDays(1, daysAgo: 0);
    ...
    test('computes current streak of 5 days ending today', () async {
      await seedConsecutiveDays(5, daysAgo: 0);
    ...
    test('current streak is broken when last completion was 2 days ago', () async {
      await seedConsecutiveDays(5, daysAgo: 2);
      
      final streaks = await service.computeStreaks(loc);
      
      expect(streaks.current, 0, reason: 'Streak should be broken after 2 days gap');
      expect(streaks.best, 5);
    });
    ...
    test('handles single day gaps correctly', () async {
      ...
      await seedConsecutiveDays(2, daysAgo: 0);
    ...
    test('missed or none status prayers do not count', () async {
      ...
      expect(streaks.current, 0, reason: 'Day with missed/none prayers should not count');
      expect(streaks.best, 0);
    });

    test('long streak of 30 days is computed correctly', () async {
      await seedConsecutiveDays(30, daysAgo: 0);
