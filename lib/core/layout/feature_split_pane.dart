...
/// Responsive entry point for Hadith, Quran study, and Fortress split layouts.
///
/// Gates on [canUseHorizontalSplit] and falls back to [stackedFallback] when the
/// container is too narrow. When split is allowed, delegates to [FeatureSplitPane]
/// (which wraps [CollapsibleHorizontalSplitPane]).
class ResponsiveFeatureSplit extends StatelessWidget {
  /// Creates a responsive feature split layout.
  const ResponsiveFeatureSplit({
  ...
  /// Layout shown when the container cannot honor horizontal split minimums.
  final Widget stackedFallback;
  ...
  /// Space subtracted from container width before [canUseHorizontalSplit].
  final double spacer;
...
///
/// Prefer [ResponsiveFeatureSplit] for the full gating + stacked-fallback pattern.
